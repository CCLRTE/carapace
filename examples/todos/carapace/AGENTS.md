...
- `check-production-boundary.ts` – positive production/Carapace source-map graph proof plus emitted production-marker scan.
...
- Install the browser integration only through `installCarapaceBrowser` in `main.tsx`; let the session own teardown. Count blocked requests and activity failures as probe violations.
